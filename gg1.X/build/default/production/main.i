...
void SRAM(char a, char b, char c, char d){
    ...
    if (a == 1) do { LATBbits.LATB3 = 1; } while(0);
    else do { LATBbits.LATB3 = 0; } while(0);
    if (b == 1) do { LATBbits.LATB2 = 1; } while(0);
    else do { LATBbits.LATB2 = 0; } while(0);
    if (c == 1) do { LATBbits.LATB1 = 1; } while(0);
    else do { LATBbits.LATB1 = 0; } while(0);
    if (d == 1) do { LATBbits.LATB0 = 1; } while(0);
...
void main(void)
{
    ...
    while (1)
    {
        ...
        I2C1_MasterRead(&level, length, 0x08, &p_status);
        while(w_status != I2C1_MESSAGE_COMPLETE);
        if (level == '0'){
          sendString("WARNING generated progress hold\n");
          command = 'f';
          I2C1_MasterWrite(&command, length, 0x08, &w_status);
        }
